...
def init_db():
    ...
    if cursor.fetchone()[0] == 0:
        try:
            with open('Data/ec_guide.json', 'r', encoding='utf-8') as f:
                guide_data = json.load(f)
                cursor.execute('INSERT INTO guides (title, content) VALUES (?, ?)', 
                             (guide_data['title'], guide_data['content']))
        except FileNotFoundError:
            logger.warning("Guide file not found, using default")
            cursor.execute('INSERT INTO guides (title, content) VALUES (?, ?)', 
                         ("Guide", "Guide content not available"))
    ...
    if cursor.fetchone()[0] == 0:
        try:
            with open('Data/ec_rules.json', 'r', encoding='utf-8') as f:
                rules_data = json.load(f)
                cursor.execute('INSERT INTO rules (title, content) VALUES (?, ?)', 
                             (rules_data['title'], rules_data['content']))
        except FileNotFoundError:
            logger.warning("Rules file not found, using default")
            cursor.execute('INSERT INTO rules (title, content) VALUES (?, ?)', 
                         ("Rules", "Rules content not available"))
        except Exception as e:
